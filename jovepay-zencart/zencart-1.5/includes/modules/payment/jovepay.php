...
class jovepay extends base
{
  ...
  function __construct()
  {
    ...
    $this->callback_url = zen_catalog_href_link(
      'jovepay_handler.php', '', 'SSL'
    );
    if (is_object($order))
      $this->update_status();
  }
  ...
  function update_status()
  {
    ...
    // $order_reference = 'ZC-' . $_SESSION['customer_id'] . '-' . time();
    $order_reference = 'ZEN-' . $order->info['orders_id'];
    $_SESSION['jovepay_reference'] = $order_reference;
    $theme = MODULE_PAYMENT_JOVEPAY_THEME == 'True' ? 'dark' : 'light';
    $isTestnet = MODULE_PAYMENT_JOVEPAY_TESTNET == 'True' ? true : false;
    $json = array();
    $json['dataSource'] = STORE_NAME;
    $json['ipnCallbackUrl'] = $this->callback_url;
    ...
    $this->form_action_url = 'https://www.jovepay.com/pay/payment/?params='
      . urlencode(base64_encode(json_encode($json)))
      . '&locale=' . $locale
      . '&theme=' . $theme
      . '&isTestnet=' . $isTestnet;
  }
  ...
  function install()
  {
    ...
    $db->Execute('insert into ' . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('JOVEpay Testnet', 'MODULE_PAYMENT_JOVEPAY_TESTNET', 'False', 'Enable Testnet mode', '6', '0', now())");
    $db->Execute('insert into ' . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('JOVEpay Theme', 'MODULE_PAYMENT_JOVEPAY_THEME', 'light', 'Enable Dark Theme', '6', '0', now())");
    $db->Execute('insert into ' . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('JOVEpay Locale', 'MODULE_PAYMENT_JOVEPAY_LOCALE', 'en', 'Locale', '6', '0', now())");
  }
  ...
  function keys()
  {
    return array(
      ...
      'MODULE_PAYMENT_JOVEPAY_TESTNET',
      'MODULE_PAYMENT_JOVEPAY_THEME',
      'MODULE_PAYMENT_JOVEPAY_LOCALE'
    );
  }
}
